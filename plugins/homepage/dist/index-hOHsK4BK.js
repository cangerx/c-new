...
}`;function Ne(o){return"paperShaderMount"in o}function Ve(){const o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function ke(o){const e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,i=window.innerWidth-o.documentElement.clientWidth,f=e*a+i,c=outerWidth/f,n=Math.round(100*c);return n%5===0?n/100:n===33?1/3:n===67?2/3:n===133?4/3:c}const We={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Ye={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Le={none:0,contain:1,cover:2},w=`
...
`,ce={maxColorCount:10},He=`#version 300 es
precision mediump float;
...
`,oe={maxColorCount:10,maxNoiseIterations:8},Xe=`#version 300 es
precision mediump float;
...
vec2 fbm(vec2 n0, vec2 n1) {
  vec2 total = vec2(0.0);
  float amplitude = .4;
  for (int i = 0; i < ${oe.maxNoiseIterations}; i++) {
...
void main() {
  ...
  for (int i = ${oe.maxColorCount} - 2; i >= 0; i--) {
...
`,ue={maxColorCount:10},je=`#version 300 es
precision mediump float;
...
`,qe={circle:0,diamond:1,square:2,triangle:3},fe={maxColorCount:10},Ze=`#version 300 es
precision mediump float;
...
`,te={maxColorCount:8,maxBallsCount:20},$e=`#version 300 es
precision mediump float;
...
`,me={maxColorCount:5},oo=`#version 300 es
precision mediump float;
...
`,pe={maxColorCount:10},ao=`#version 300 es
precision mediump float;
...
`,io={checks:0,stripes:1,edge:2},de={maxColorCount:5},ro=`#version 300 es
precision mediump float;
...
`,co={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},uo={random:1,"2x2":2,"4x4":3,"8x8":4},ve={maxColorCount:7},fo=`#version 300 es
...
`,mo={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7},ae={maxColorCount:5,maxSpots:4},po=`#version 300 es
precision lowp float;
...
void main() {
  ...
  for (int colorIdx = 0; colorIdx < ${ae.maxColorCount}; colorIdx++) {
    ...
    for (int spotIdx = 0; spotIdx < ${ae.maxSpots}; spotIdx++) {
  ...
  fragColor = vec4(color, opacity);
}`,go={auto:0,square:1},ie={maxColorCount:7},vo=`#version 300 es
precision lowp float;
...
void main() {
  ...
  vec4 premultipliedColors[${ie.maxColorCount}];
  for (int i = 0; i < ${ie.maxColorCount}; i++) {
...
`,xe={maxColorCount:10},xo=`#version 300 es
precision mediump float;
...
`,bo={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},Co={prism:1,lens:2,contour:3,cascade:4,flat:5},wo=`#version 300 es
precision mediump float;
...
`,_e={maxColorCount:10},Bo=`#version 300 es
...
`;function So(o){const e=document.createElement("canvas"),a=1e3;return new Promise((i,f)=>{const c=new Image;c.crossOrigin="anonymous",c.addEventListener("load",()=>{(typeof o=="string"?o.endsWith(".svg"):o.type==="image/svg+xml")&&(c.width=a,c.height=a);const n=c.naturalWidth/c.naturalHeight,t=Math.floor(a*.15),m=Math.ceil(t*2.5);let s=a,u=a;n>1?u=Math.floor(a/n):s=Math.floor(a*n),e.width=s+2*m,e.height=u+2*m;const p=e.getContext("2d",{willReadFrequently:!0});if(!p)throw new Error("Failed to get canvas 2d context");p.fillStyle="white",p.fillRect(0,0,e.width,e.height),p.drawImage(c,m,m,s,u);const{width:b,height:k}=e,r=p.getImageData(0,0,b,k).data,l=b*k,v=new Uint8ClampedArray(l);for(let _=0;_<l;_++){const M=_*4,Y=r[M]??0,T=r[M+1]??0,H=r[M+2]??0;v[_]=.299*Y+.587*T+.114*H|0}const C=t,S=Math.max(1,Math.round(.12*t)),B=5,g=re(v,b,k,C,3),A=re(v,b,k,S,3),R=re(v,b,k,B,1),P=p.createImageData(b,k),Q=P.data;for(let _=0;_<l;_++){const M=_*4;Q[M]=R[_]??0,Q[M+1]=g[_]??0,Q[M+2]=A[_]??0,Q[M+3]=255}p.putImageData(P,0,0),e.toBlob(_=>{if(!_){f(new Error("Failed to create PNG blob"));return}i({blob:_})},"image/png")}),c.addEventListener("error",()=>{f(new Error("Failed to load image"))}),c.src=typeof o=="string"?o:URL.createObjectURL(o)})}function ye(o,e,a,i){if(i<=0)return o.slice();const f=new Uint8ClampedArray(e*a),c=new Uint32Array(e*a);for(let n=0;n<a;n++){let t=0;for(let m=0;m<e;m++){const s=n*e+m,u=o[s]??0;t+=u,c[s]=t+(n>0?c[s-e]??0:0)}}for(let n=0;n<a;n++){const t=Math.max(0,n-i),m=Math.min(a-1,n+i);for(let s=0;s<e;s++){const u=Math.max(0,s-i),p=Math.min(e-1,s+i),b=m*e+p,k=m*e+(u-1),y=(t-1)*e+p,r=(t-1)*e+(u-1),l=c[b]??0,v=u>0?c[k]??0:0,C=t>0?c[y]??0:0,S=u>0&&t>0?c[r]??0:0,B=l-v-C+S,g=(p-u+1)*(m-t+1);f[n*e+s]=Math.round(B/g)}}return f}function re(o,e,a,i,f){if(i<=0||f<=1)return ye(o,e,a,i);let c=o,n=o;for(let t=0;t<f;t++)n=ye(c,e,a,i),c=n;return n}const Uo=`#version 300 es
precision mediump float;
...
`,Ro={classic:0,gooey:1,holes:2,soft:3},Fo={square:0,hex:1},Mo=`#version 300 es
precision mediump float;
...
`,Eo={dots:0,ink:1,sharp:2},Ae={maxColorCount:6},zo=`#version 300 es
precision mediump float;
