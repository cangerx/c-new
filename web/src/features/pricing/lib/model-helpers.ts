...
/** Check whether this model uses the dedicated fixed video-task price. */
export function isVideoPerRequestModel(model: PricingModel): boolean {
  return model.billing_mode === 'video_per_request'
}

/** Resolve the fixed price that the backend actually uses for this request mode. */
export function getFixedRequestPrice(model: PricingModel): number {
  const price = isVideoPerRequestModel(model)
    ? model.video_model_price
    : model.model_price
