...
type Pricing struct {
	...
	VideoModelPrice        *float64                `json:"video_model_price,omitempty"`
...
func updatePricing() {
	...
	for model, groups := range modelGroupsMap {
		...
		videoModelPrice, findVideoPrice := ratio_setting.GetVideoModelPrice(model, false)
		modelPrice, findPrice := ratio_setting.GetModelPrice(model, false)
		if findVideoPrice {
			pricing.VideoModelPrice = &videoModelPrice
			pricing.QuotaType = 1
			pricing.BillingMode = "video_per_request"
		} else if findPrice {
